...
function extractTopic(data: Record<string, unknown>, sourceContext: string): string {
...
function buildPrompt(data: Record<string, unknown>, topic: string, sourceContext: string): string {
  const parts = [
...
function createSectionPlans(questionType: unknown, totalQuestions: number) {
  const type = normaliseText(questionType).toLowerCase()

  if (type === "mixed") {
    const sectionCounts = allocateCounts(totalQuestions, [4, 3, 3])
    return [
      { name: "Section A", instructions: "Attempt all questions.", questionType: "mcq", difficulty: "easy", count: sectionCounts[0] },
      { name: "Section B", instructions: "Attempt all questions.", questionType: "short", difficulty: "medium", count: sectionCounts[1] },
      { name: "Section C", instructions: "Attempt all questions.", questionType: "long", difficulty: "hard", count: sectionCounts[2] },
  ...
  if (type === "short") {
    ...
    return sectionCounts.length === 1
      ? [{ name: "Section A", instructions: "Attempt all questions.", questionType: "short", difficulty: "medium", count: sectionCounts[0] }]
      : [
          { name: "Section A", instructions: "Attempt all questions.", questionType: "short", difficulty: "easy", count: sectionCounts[0] },
          { name: "Section B", instructions: "Attempt all questions.", questionType: "short", difficulty: "medium", count: sectionCounts[1] },
        ].filter((section) => section.count > 0)
  }
  ...
  if (type === "long") {
    ...
    return sectionCounts.length === 1
      ? [{ name: "Section A", instructions: "Attempt all questions.", questionType: "long", difficulty: "hard", count: sectionCounts[0] }]
      : [
          { name: "Section A", instructions: "Attempt all questions.", questionType: "long", difficulty: "medium", count: sectionCounts[0] },
          { name: "Section B", instructions: "Attempt all questions.", questionType: "long", difficulty: "hard", count: sectionCounts[1] },
        ].filter((section) => section.count > 0)
  }

  return [{ name: "Section A", instructions: "Attempt all questions.", questionType: type === "true-false" ? "true-false" : "mcq", difficulty: "easy", count: totalQuestions }]
}
...
function allocateMarks(totalMarks: number, questionCount: number) {
  if (questionCount <= 0) return []
  const base = Math.floor(totalMarks / questionCount)
  const remainder = totalMarks - base * questionCount
  return Array.from({ length: questionCount }, (_value, index) => base + (index < remainder ? 1 : 0))
}

function buildQuestionText(questionType: string, topic: string, index: number, sourceContext: string) {
...
export function generateStructuredPaper(data: Record<string, unknown>) {
  ...
  const sections = sectionPlans.map((sectionPlan, sectionIndex) => {
    ...
    const questions = Array.from({ length: sectionPlan.count }, (_value, questionIndex) => {
      const questionMarks = marksAllocation[questionIndex] || 1
      const difficulty = sectionPlan.difficulty || (questionIndex % 3 === 0 ? "easy" : questionIndex % 3 === 1 ? "medium" : "hard")
