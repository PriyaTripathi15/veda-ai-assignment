...
import { generateStructuredPaper } from "./aiService"

export async function processAssignment(assignmentId: string) {
  const assignment: any = await Assignment.findById(assignmentId)
...
export async function failAssignment(assignmentId: string, error: Error) {
  const assignment: any = await Assignment.findById(assignmentId)
